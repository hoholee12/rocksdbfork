  ...
  void Run() {
    ...
    while (std::getline(benchmark_stream, name, ',')) {
      ...
      if (name == "fillseqdeterministic" ||
        ...
        } else if (name == "ycsbwkldload") {
			method = &Benchmark::YCSBWorkloadLoad;    /////
...
// optimization, so skip the check of the max level.
  ...
  void YCSBWorkloadLoad(ThreadState* thread) {
#if 0
		if( thread->tid == thread->shared->total - 1 ) {
		    //record latency and throughput per second
		...
		while (!duration.Done(1)) {
			...
			per_op_start_time = FLAGS_env->NowNanos();
		    }
			...
			thread->shared->latencys_mutex.Lock();
			thread->shared->latencys[thread->shared->ops_num] = FLAGS_env->NowNanos() - per_op_start_time;
		...
		if( thread->tid == thread->shared->total - 1 ) {
			...
			if (per_second_done > 2) {
			    uint64_t cnt90 = 0.90 * per_second_done - 1 + last_ops;
			    uint64_t cnt99 = 0.99 * per_second_done - 1 + last_ops;
			    uint64_t cnt995 = 0.995 * per_second_done - 1 + last_ops;
			    uint64_t cnt999 = 0.999 * per_second_done - 1 + last_ops;
			    uint64_t cnt9999 = 0.9999 * per_second_done - 1 + last_ops;
			    uint64_t cnt99999 = 0.99999 * per_second_done - 1 + last_ops;
			    ...
			    RECORD_INFO(6,"%.2f,%.1f,%lu,,,%lu,,,%lu,,,%lu,,,%lu,,,%lu,,,\n",
				    now,1.0*per_second_done/use_time,
				    ops_latency[cnt90],
				    ops_latency[cnt99],
				    ops_latency[cnt995],
		...
		while (!duration.Done(1)) {
		    ...
		    if (FLAGS_report_ops_latency) {   //
			per_op_start_time = FLAGS_env->NowNanos(); //FLAGS_env->NowMicros();
		    }
			...
			if (FLAGS_report_ops_latency) {   //
			    per_op_start_time = FLAGS_env->NowNanos(); //NowMicros();
			}
			...
			thread->shared->latencys[thread->shared->ops_num] = FLAGS_env->NowNanos() - per_op_start_time;
			thread->shared->ops_num++;
			if(thread->shared->ops_num == 10000000)
			    thread->shared->ops_num = 0;
			thread->shared->latencys_mutex.Unlock();
		    }
	    ...
	    // Workload B: Update heavy workload
	    // This workload has a mix of 5/95 reads and writes. 
	    // An application example is a session store recording recent actions.
	    ...
	    // Workload C: 
	    // This workload has a mix of 100/0 reads and writes. 
	    // An application example is a session store recording recent actions.
	    // Read/update ratio: read 100
	    // Default data size: 1 KB records 
	    // Request distribution: zipfian
		...
		if (FLAGS_benchmark_write_rate_limit > 0) {
		    printf(">>>> FLAGS_benchmark_write_rate_limit YCSBC \n");
		    thread->shared->write_rate_limiter.reset(
			    NewGenericRateLimiter(FLAGS_benchmark_write_rate_limit));
		}
		...
		while (!duration.Done(1)) {
		    ...
		    if (FLAGS_report_ops_latency) {   //
			per_op_start_time = FLAGS_env->NowNanos();
			//thread->shared->latencys[thread->shared->ops_num] = FLAGS_env->NowMicros();
		    }
